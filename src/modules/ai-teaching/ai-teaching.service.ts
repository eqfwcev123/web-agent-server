...
export class AiTeachingService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (apiKey) {
      ...
      this.logger.warn('OpenAI API key not configured. AI features will use mock responses.');
    }
  }
    ...
    try {
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert programming tutor. Provide helpful, educational hints that guide students toward the solution without giving it away completely.',
          },
          {
            role: 'user',
            content: prompt,
      ...
      const content = completion.choices[0]?.message?.content || 'Unable to generate hint';
    ...
    try {
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert programming educator. Explain programming concepts clearly with examples, analogies, and practical applications.',
          },
          {
            role: 'user',
            content: prompt,
      ...
      const content = completion.choices[0]?.message?.content || 'Unable to explain concept';
    ...
    try {
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert code reviewer. Provide constructive feedback on code quality, efficiency, readability, and potential improvements. Be encouraging and educational.',
          },
          {
            role: 'user',
            content: prompt,
      ...
      const content = completion.choices[0]?.message?.content || 'Unable to review code';
    ...
    try {
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a supportive programming mentor. Provide encouraging, specific feedback that helps students learn and improve. Focus on both strengths and areas for growth.',
          },
          {
            role: 'user',
            content: prompt,
      ...
      const content = completion.choices[0]?.message?.content || 'Unable to provide feedback';
...
`;
  ...
  private buildFeedbackPrompt(request: AiRequestDto): string {
    const { problemContext, userCode, sessionContext, executionResult } = request;
...
`;
  ...
  private getMockResponse(type: string, startTime: number): AiResponseDto {
    const mockResponses = {
      ...
      explanation: "This concept involves understanding how to efficiently search and store data. Think about the relationship between keys and values.",
      code_review: "Your code shows good understanding of the basic approach. Consider edge cases and think about time complexity optimization.",
      feedback: "Great effort on this problem! You're making good progress. Keep practicing similar problems to strengthen your understanding.",
    };
