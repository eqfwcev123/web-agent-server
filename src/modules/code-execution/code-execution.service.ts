...
export class CodeExecutionService {
	...
	constructor(
		...
		this.judge0BaseUrl =
			this.configService.get<string>('codeExecution.judge0Url') ||
			'https://judge0-ce.p.rapidapi.com';
		this.judge0ApiKey = this.configService.get<string>(
			'codeExecution.judge0ApiKey',
		);
	}
		...
		try {
			...
			if (!languageId) {
				throw new BadRequestException(
					`Unsupported language: ${executeDto.language}`,
				);
			}
	...
	async validateSolution(
		...
		try {
			...
			return {
				...
				error:
					results.find((r) => !r.passed && !r.isHidden)
						?.actualOutput || null,
				executionTime: Math.max(
					...results.map((r) => r.executionTime || 0),
				),
